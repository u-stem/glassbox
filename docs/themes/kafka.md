...
- **ブローカーを停止して再開すると、一部の consumer は消費を再開しない**: ホーム画面からブローカーを停止・再起動した場合、admin poller は自動で `connected` に戻り(実測で 3〜6 秒)、producer も次の送信で再接続する。一方 consumer は、消費ストリームが切断で reject するとそのまま再試行しない(`apps/gateway/src/themes/kafka/actors/consumer-actor.ts` のクラスコメント "does not retry automatically")。どの consumer がそうなるかは決定的ではなく、実測では「2 台とも脱落」「1 台だけ脱落」の両方が起きた。
  - gateway は `onConsumeError` でこれを検知し、該当 consumer を kill モードで撤去して `actor.destroyed` を publish する。UI からは消え、`MAX_CONSUMERS` の枠も解放される。撤去後はブローカーがセッションタイムアウト(既定 60 秒)で member を期限切れにし、リバランスが走って**生き残った consumer が全パーティションを引き継ぐ**(実測: 引き継ぎ後にラグ 0 まで消費が進むところまで確認)。観察台数を戻すには consumer を追加し直す。
  - 撤去に graceful ではなく kill を使うのは実測に基づく。graceful は LeaveGroup を送ろうとするが停止したブローカーには届かず、その後も `@platformatic/kafka` の `Consumer` がメンバーシップを維持し続けてしまった(ブローカーは数分後も STABLE の member として全パーティションを割り当てたまま報告し、gateway プロセスを再起動するまで解消しなかった)。kill は接続を破棄するため、セッションタイムアウトによる期限切れが確実に働く。
- **撤去した consumer がブローカー側の group から消えるまで最大 60 秒かかる**: 上記のセッションタイムアウト待ちのあいだ、admin snapshot には member として表示され続ける。ブローカーが権威であるという [ADR-0003](../adr/0003-state-authority-model.md) の方針どおりの挙動で、待てば収束する。
